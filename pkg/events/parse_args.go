	...
	case BpfAttach:
		...
		if helpersArg := GetArg(event, "prog_helpers"); helpersArg != nil {
			if helpersList, isUintSlice := helpersArg.Value.([]uint64); isUintSlice {
				parsedHelpersList, err := parseBpfHelpersUsageOld(helpersList)
				if err != nil {
					return err
				}
		...
		if attachTypeArg := GetArg(event, "attach_type"); attachTypeArg != nil {
			if attachType, isInt := attachTypeArg.Value.(int32); isInt {
				attachTypestr, err := parseBpfAttachTypeOld(attachType)
...
func parseBpfHelpersUsageOld(helpersList []uint64) ([]string, error) {
...
func parseBpfAttachTypeOld(attachType int32) (string, error) {
	switch attachType {
	case 0:
