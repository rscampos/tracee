...
func (ps *policies) createNewDataFilterMapsVersion(
	...
	mapsNames := map[string]struct {
		outerMapName string
		equalities   map[filters.KernelDataFields]equality
	}{
...
func (ps *policies) updateStringDataFilterLPMBPF(dataEqualities map[filters.KernelDataFields]equality, innerMapName string) error {
...
func (ps *policies) updateStringDataFilterBPF(dataEqualities map[filters.KernelDataFields]equality, innerMapName string) error {
	// KernelDataFields equality
...
func (ps *policies) updateBPF(
	...
	fEqs := &filtersEqualities{
		...
		dataEqualitiesPrefix: make(map[filters.KernelDataFields]equality),
		dataEqualitiesSuffix: make(map[filters.KernelDataFields]equality),
		dataEqualitiesExact:  make(map[filters.KernelDataFields]equality),
...
func (ps *policies) computePoliciesConfig() *PoliciesConfig {
	...
	for _, p := range ps.allFromMap() {
		...
		if p.DataFilter.EnabledExcatMatch() {
		...
		if p.DataFilter.MatchIfKeyMissingExcatMatch() {
			cfg.DataFilterExactMatchIfKeyMissing |= 1 << offset
		}
		if p.DataFilter.MatchIfKeyMissingPrefixMatch() {
			cfg.DataFilterPrefixMatchIfKeyMissing |= 1 << offset
		}
		if p.DataFilter.MatchIfKeyMissingSuffixMatch() {
