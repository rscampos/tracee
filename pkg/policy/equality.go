...
type filtersEqualities struct {
	...
	dataEqualitiesPrefix map[filters.KernelDataFilter]equality
	dataEqualitiesSuffix map[filters.KernelDataFilter]equality
	dataEqualitiesExact  map[filters.KernelDataFilter]equality
...
func updateAffixEqualities[T comparable](
	...
	for newK := range filterEqualities {
		...
		newKD, isKernelData := any(newK).(filters.KernelDataFilter)

		var longestMatch filters.KernelDataFilter
		var longestMatchEq equality

		if isKernelData {
			for existingK, existingEq := range equalitiesMap {
				existingKD, isExistingKernelData := any(existingK).(filters.KernelDataFilter)
...
func (ps *policies) computeFilterEqualities(
	...
	for _, p := range ps.allFromMap() {
		...
		updateEqualities(fEqs.dataEqualitiesExact, dataEqualities.NotEqual, notEqual, policyID)
		updateEqualities(fEqs.dataEqualitiesExact, dataEqualities.Equal, equal, policyID)
