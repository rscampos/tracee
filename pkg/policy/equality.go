...
type filtersEqualities struct {
	...
	dataEqualitiesPrefix map[filters.KernelDataFields]equality
	dataEqualitiesSuffix map[filters.KernelDataFields]equality
	dataEqualitiesExact  map[filters.KernelDataFields]equality
...
func updateAffixEqualities[T comparable](
	...
	for newK := range filterEqualities {
		...
		newKD, isKernelData := any(newK).(filters.KernelDataFields)

		var longestMatch filters.KernelDataFields
		var longestMatchEq equality

		if isKernelData {
			for existingK, existingEq := range equalitiesMap {
				existingKD, isExistingKernelData := any(existingK).(filters.KernelDataFields)
...
func (ps *policies) computeFilterEqualities(
	...
	for _, p := range ps.allFromMap() {
		...
		updateEqualities(fEqs.dataEqualitiesExact, dataEqualities.ExactNotEqual, notEqual, policyID)
		updateEqualities(fEqs.dataEqualitiesExact, dataEqualities.ExactEqual, equal, policyID)

		updateAffixEqualities(fEqs.dataEqualitiesPrefix, dataEqualities.PrefixNotEqual, notEqual, policyID)
		updateAffixEqualities(fEqs.dataEqualitiesPrefix, dataEqualities.PrefixEqual, equal, policyID)

		updateAffixEqualities(fEqs.dataEqualitiesSuffix, dataEqualities.SuffixNotEqual, notEqual, policyID)
		updateAffixEqualities(fEqs.dataEqualitiesSuffix, dataEqualities.SuffixEqual, equal, policyID)
